...
function Sitemapsubcategory() {
  const [reports, setReports] = useState([]);

  useEffect(() => {
    const fetchReports = async () => {
      try {
        const response = await axios.get('https://app-node.enterprisetalk.com:3000/api/category/sitemap-cat-list'); // Assuming you have an endpoint to fetch reports
      ...
      <urlset {...namespaceAttrs}>
        {reports.map(report => (
          <ul key={report.id} className='mt-3'>
            <url>
              <li style={{fontSize:"13px", listStyleType:"none", fontWeight:"500" }}> <a href={`${base_url + 'topic' + "/" + report.cat_slug}`} className='text-black'>{base_url + 'topic' + "/" + report.cat_slug}</a></li>
            </url>
...
export default Sitemapsubcategory;
